...
/**
 * 一键安装推荐 Skills
 * 调用 skills/install.sh 脚本，自动跳过已安装
 */
function cmdSkills() {
  const scriptPath = path.join(__dirname, 'skills', 'install.sh');
  if (!fs.existsSync(scriptPath)) {
    console.log(`${c.error('❌')} skills/install.sh 未找到`);
    return;
  }
  try {
...
async function main() {
  ...
  if (args.command === 'skills') return cmdSkills();
...
${COLORS.cyan}║${COLORS.reset}    skills      一键安装推荐 Skills 合集
